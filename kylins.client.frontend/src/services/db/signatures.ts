...
export async function getDefaultSignature(accountId: string): Promise<DbSignature | null> {
...
export async function insertSignature(sig: {
  ...
  if (sig.isDefault) {
    await db.execute('UPDATE signatures SET is_default = 0 WHERE account_id = $1', [sig.accountId]);
  }
...
export async function updateSignature(
  ...
  if (updates.isDefault) {
    ...
    if (rows[0]) {
      await db.execute('UPDATE signatures SET is_default = 0 WHERE account_id = $1', [
        rows[0].account_id,
      ]);
    }
  }
